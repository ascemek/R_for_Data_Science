...
1) What does str_sub() do? Give an example of when this might be helpful.
2) What does str_trim() do? What’s the opposite of str_trim()?
3) How can you tell how many characters are in a string?

1) str_sub() extracts substrings from a vector. It will recycle all arguments to be the same length as the longest argument. If any arguments are of length 0, the output will be a zero length character vector. This might be helpful when we want to shorthen the length of a string or access the certain part of a string.
Ex: str_sub(fruit,1,3); str_sub(fruit,-2)

2) str_trim() trims whitespace from the start and/or end of a string. The opposite of str_trim() is str_pad() which adds characters to each side. Ex: str_trim(fruit)

3) We can use one of the following functions to find how many characters are in a string: Using str_length() method, using nchar() method, and using stri_length() method.
...
```{r}
(b_fruits <- str_subset(fruit, pattern = "b"))
```
...
```{r}
(ly_words <- str_subset(words, pattern = "ly"))
```
...
```{r}
#We have a vector that has the name and last name separated by comma. We want to have a separate column for for name and last name.

full_names

str_split_fixed(full_names, pattern = ", ", n = 2)
```
...
```{r}
full_names_df <- tibble(emails)
full_names_df %>% 
  separate(emails, into = c("name"), sep = "@hogwarts.edu")
```
...
ingredients_words <- str_split(ingredients, pattern = ", ")

ingredients_words

#sortiing alphabetically didn't work
#sort(ingredients_words)
...
```{r}
average_sent_len <- str_length(sentences)
...
The difference between sep and collapse arguments in the str_c() function is:
- The output format for sep is characters(string) but for collapse the format is character vector of length one.
- For sep chars are separeted by comma even though we combine the vectors but for collapse chars become together and make a new element in the list/vector. 
